...
# 构建当前版本
cd src && go build -ldflags="-s -w -X main.version=v0.0.9" -o /tmp/dbexplain-curr .
...
| 功能 | 版本引入 | 验证方式 |
|------|---------|---------|
| Schema 采集 (全部类型) | v0.0.4+ | test/02 |
| 跨库关系推断 | v0.0.4+ | test/02 |
| JSON/--human 输出 | v0.0.4+ | test/02 |
| -include/-exclude 过滤 | v0.0.5+ | test/02 |
| --context AI 上下文 | v0.0.5+ | test/10 |
| -cache 增量扫描 | v0.0.5+ | test/10 |
| 多 DSN 并发采集 | v0.0.5+ | test/02 |
| execute 只读查询 | v0.0.7+ | test/03, 04, 05 |
| sqlguard 三层校验 | v0.0.7+ | test/06 |
| 并发控制 | v0.0.7+ | test/08 |
| 安全策略引擎 | v0.0.8+ | test/07 |
| CSV/TSV/XLSX 文件 | v0.0.9 | test/05 |
| 单二进制架构 | v0.0.9 | test/01 |
| CONFIG_SEARCH 文档 | v0.0.9 | — |
...
| 数据源数 | v0.0.8 耗时 | v0.0.9 预期 |
|---------|-----------|------------|
| 9 DSN | ~15s | — |
| 15 DSN | — | < 60s |

> v0.0.9 新增 6 个数据源（CSV/TSV/XLSX），文件处理为纯本地操作，新增耗时可忽略。主要耗时仍在远程数据库连接上。
