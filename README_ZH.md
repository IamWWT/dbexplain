...
`dbexplain` 是一个**单二进制、零运行时依赖**的命令行工具，支持 **17 种异构数据源**（含可选 DuckDB）的 Schema 采集与只读查询执行，所有操作在统一的安全沙箱下审计可追溯。
...
| **连接器层** | 17 种数据源统一接口 | `connector/` — 每数据源独立文件，`init()` 自注册到全局 registry |
...
|:----:|------|:--------:|:-----------:|:--------:|
| L1 | **sqlguard** — AST 只读校验（8 读/17 写动词，EXPLAIN 内部语句递归校验） | ✅ | — | — |
