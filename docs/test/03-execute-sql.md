...
```bash
$BIN execute --db 1 "SELECT 1 AS n, 'hello' AS s" --human
```
...
```bash
$BIN execute --db 1 "SELECT hostip, device_type FROM testdb.iplist LIMIT 5" --human
```

## 3.3 PostgreSQL — 基本查询

```bash
$BIN execute --db 6 "SELECT 1" --human
```

## 3.4 PostgreSQL — 表关联

```bash
$BIN execute --db 6 --human \
...
```bash
$BIN execute --db 2 "SELECT 1 AS n" --human
```

## 3.6 ClickHouse — 聚合查询

```bash
$BIN execute --db 2 --human \
...
```bash
$BIN execute --db 3 "SELECT 1" --human
```

## 3.8 SQLite — 聚合

```bash
$BIN execute --db 3 --human \
...
```bash
$BIN execute --db 5 "SHOW TABLES" --human
```

## 3.10 Elasticsearch — 表查询

```bash
$BIN execute --db 5 "SELECT title, severity FROM runbooks LIMIT 5" --human
```

## 3.11 EXPLAIN 查询计划

```bash
$BIN execute --db 1 --explain "SELECT * FROM testdb.iplist WHERE device_type = 'PHY'" --human
```

## 3.12 自定义超时和行数

```bash
$BIN execute --db 6 --timeout 60 --limit 500 "SELECT * FROM public.video_descriptions" --human
```
...
$BIN execute --dsl "SELECT i.hostip, i.device_type, r.type AS rule_type
