...
```bash
DBPROBE_ENV_FILE=../testdata/qa/.env.qa-touch-csv $BIN execute --label touch-ops-csv \
...
```bash
DBPROBE_ENV_FILE=../testdata/qa/.env.qa-touch-join $BIN execute --label touch-ops \
...
```bash
DBPROBE_ENV_FILE=../testdata/qa/.env.qa-security $BIN execute --label touch-ops 'SELECT *' --limit 1
...
```bash
DBPROBE_ENV_FILE=../testdata/qa/.env.qa-security $BIN execute --label sec-org 'SELECT *' --limit 3 --human
...
DBPROBE_ENV_FILE=../testdata/qa/.env.qa-security $BIN execute --label touch-ops 'DROP TABLE pb_touch_ops_sample_2000'
# → ACCESS_DENIED: table "pb_touch_ops_sample_2000" is not allowed for query

# 对未禁止表执行 DROP → 文件查询引擎拒绝非 SELECT
DBPROBE_ENV_FILE=../testdata/qa/.env.qa-security $BIN execute --label sec-org 'DROP TABLE t_sec_org_sample'
