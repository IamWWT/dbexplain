...
| `row_count` | 行数归一化（评分因子） | MySQL, PostgreSQL, GaussDB, ClickHouse, SQLite, MongoDB, Qdrant, CSV |
| `index` | 索引密度（评分因子） | MySQL, PostgreSQL, GaussDB, SQLite |
| `vector` | 向量集合元数据 | Qdrant |
| `file` | 文件数据源只读查询 | CSV/TSV, XLSX |
| `promql` | PromQL 查询（Prometheus） | Prometheus |
...
| 属性 | 说明 |
|---|---|
| **文件** | `src/internal/analyze/analyze.go` → `inferRefs()` |
| **依赖包** | `strings`（标准库） |
...
| 属性 | 说明 |
|---|---|
| **文件** | `src/internal/analyze/analyze.go` → `clusterGroups()` |
| **依赖包** | `strings`（标准库） |
| **可替换** | 是。可替换为其他图连通分量算法 |
...
| 属性 | 说明 |
|---|---|
| **文件** | `src/internal/schema/infer.go` → `InferComment()` |
| **依赖包** | `strings`（标准库） |
...
| 属性 | 说明 |
|---|---|
| **文件** | `src/internal/analyze/ranking.go` → `Ranker.Rank()` |
| **依赖包** | `math`, `sort`（标准库） |
...
| 属性 | 说明 |
|---|---|
| **文件** | `src/internal/connector/redis.go` → `normalize()` |
| **依赖包** | `regexp`（标准库） |
...
| 属性 | 说明 |
|---|---|
| **文件** | `src/internal/cache/cache.go` |
...
| 属性 | 说明 |
|---|---|
| **文件** | `src/internal/diagnostics/diagnostics.go` |
| **依赖包** | `strings`, `fmt`（标准库） |
...
| v0.1.0 | 9 | 新增 `vector`（Qdrant 向量集合）、`file`（CSV/TSV/XLSX 文件数据源）Capability |
| v0.1.4 | 10 | 新增 `promql`（Prometheus PromQL 查询）Capability |
