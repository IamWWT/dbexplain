...
- **SQLGuard 动词白名单**：所有查询经过 `sqlguard` 模块校验，仅允许 `SELECT`、`EXPLAIN`、`WITH`、`SHOW`、`DESCRIBE`、`DESC`、`PRAGMA`、`CHECK` 八类只读动词通过。任何写操作语句将被拒绝。`EXPLAIN` 包裹的写语句（含 `EXPLAIN ANALYZE`——PostgreSQL 会真实执行）同样被递归校验拒绝（v0.1.11+）。
